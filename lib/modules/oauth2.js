...
everyModule.submodule('oauth2')
  ...
  .get('entryPath',
    ...
    .step('setDomain')
      .description('sets the domain for dynamic domains - do not override to take oauthHost')
  ...
  .setDomain ( function(req, res, next) {
    //This is only to be overridden to dynamically set the oAuthHost
...
oauth2.cloneOnSubmodule.push('moreAuthQueryParams', 'moreAccessTokenParams', 'moreAccessTokenQueryParams');
...
oauth2.accessTokenQueryParam = function (key, val) {
  if (arguments.length === 1 && key.constructor == Object) {
    for (var k in key) {
      this.accessTokenQueryParam(k, key[k]);
    }
    return this;
  }
  if (val)
    this.moreAccessTokenQueryParams[key] = val;
  return this;
};
