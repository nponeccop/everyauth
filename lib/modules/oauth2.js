...
everyModule.submodule('oauth2')
  ...
  .getAccessToken( function (code, data) {
    ...
    if(additionalQueryParams) for (var j in additionalQueryParams) {
      ...
      //Add this to the query of the post url - j is the key and param the data
      specialUrl += '?' + j + '=' + param;
    }
    ...
    request[this._accessTokenHttpMethod](opts, function (err, res, body) {
      ...
      if (parseInt(res.statusCode / 100) != 2) return p.fail({statusCode: res.statusCode, data: body});
