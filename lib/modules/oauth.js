...
everyModule.submodule('oauth')
  ...
  .get('entryPath',
    ...
    .step('setDomain')
      .description('sets the domain for dymamic domains - do not override to take oauthHost')
      .accepts('req res next')
      .promises(null)
      .canBreakTo('authCallbackErrorSteps')
    .step('initializeOAuth')
      .description('This step initializes the oauth module')
      .accepts('req res next')
      .promises(null)
  ...
  .get('callbackPath',
    ...
    .step('extractTokenAndVerifier')
      ...
      .accepts('req res next')
      .promises('requestToken verifier realmId')
    ...
    .step('getAccessToken')
      ...
      .accepts('requestToken requestTokenSecret verifier realmId')
  ...
  .setDomain ( function(req, res, next) {
    //This is only to be overriden to dynamically set the oAuthHost
    //To override, copy and uncomment the line below
    //var p = this.Promise();
    //this._oauthHost = 'yourvalue';
    //p.fulfill();
    return;
  })

  .initializeOAuth (function (req, res, next) {
    //If we had an initialization - delete it
    if (this.oauth)
      delete this.oauth;
  ...
  .extractTokenAndVerifier( function (req, res, next) {
    ...
    sess.save( function (err) {
      ...
      promise.fulfill(requestToken, verifier, realmId);
    });
    return promise;
  })
  ...
  .getAccessToken( function (reqToken, reqTokenSecret, verifier, realmId) {
